...
'''
Culebra 3D para Kivy + Python 2.7
04-02-2018
Daniel Cortes

Falta:
Parece haber un problema con Clock en Windows. consultar en web
Seleccionar username y velocidad (teclado, menu contextual y/o menu aparte)
'''
...
class BoardGame(Widget):
  ...
  def __init__(self,**kwargs):
    ...
    self.sound_eat = SoundLoader.load('brush.wav')
...
class Game(Widget):
  ...
  def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
    if keycode[1] == 'd' and self.direction != [-1,0,0]:
      ...
      if self.velocity < 10: self.velocity += 0.2
      self.pause()
      self.pause()
    elif keycode[1] == '-':
      if self.velocity > 0.3: self.velocity -= 0.2	
      self.pause()
      self.pause()
